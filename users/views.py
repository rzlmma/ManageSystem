...
from common.utils import require_role, ServerError, my_render, pages, defend_attack, http_success, http_error
from .models import *
...
def group_add(request):
    ...
    return my_render('users/group_add.html', locals(), request)
...
def group_list(request):
    ...
    return my_render('users/group_list.html', locals(), request)
...
def group_edit(request):
    ...
    return my_render('users/group_edit.html', locals(), request)
...
def user_add(request):
    ...
    return my_render('users/user_add.html', locals(), request)
...
def user_list(request):
    ...
    return my_render('users/user_list.html', locals(), request)
...
def user_detail(request):
    ...
    return my_render('users/user_detail.html', locals(), request)
...
def forget_password(request):
    if request.method == 'POST':
        ...
        if user:
            ...
            msg = u"""
            Hi %s, 请点击下面链接重设密码！
            %s/users/password/reset/?uuid=%s&timestamp=%s&hash=%s
            """ % (user.name, '', user.uuid, timestamp, hash_encode)
...
def reset_password(request):
    ...
    action = '/users/password/reset/?uuid=%s&timestamp=%s&hash=%s' % (uuid_r, timestamp, hash_encode)
    ...
    if request.method == 'POST':
        ...
        return render_to_response('users/reset_password.html', locals())
...
def user_edit(request):
    ...
    if request.method == 'GET':
        ...
        if email_need:
            ...
            """ % (user.name, '', user.username, password, user_role.get(role_post, u''))
            send_mail('您的信息已修改', msg, MAIL_FROM, [email], fail_silently=False)

        return HttpResponseRedirect(reverse('user_list'))
    return my_render('users/user_edit.html', locals(), request)
...
def profile(request):
    ...
    return my_render('users/profile.html', locals(), request)
...
def change_info(request):
    ...
    return my_render('users/change_info.html', locals(), request)
